...
from deepchem.models.losses import Loss, L2Loss, SparseSoftmaxCrossEntropy, KLLoss, SmoothKLLoss
from deepchem.models.torch_models.torch_model import TorchModel
from typing import Optional


class GCNAngle(nn.Module):
    ...
    def forward(self, g):
        ...
        * When self.mode = 'classification', the output consists of probabilities
        ...
        if self.mode == 'classification':
            ...
            output = torch.nn.functional.softplus(out)
            proba = output / output.sum(dim=-1, keepdim=True)
            return proba, out


class GCNModelAngle(TorchModel):
    """Model for Graph Property Prediction Based on Graph Convolution Networks (GCN).
    ...
    * There are various minor differences in using dropout, skip connection and batch
    ...
    def __init__(self,
        ...
        """
        model = GCNAngle(n_tasks=n_tasks,
        ...
        if mode == 'regression':
            loss: Loss = SmoothKLLoss()
            output_types = ['prediction','loss']
        else:
            loss = SparseSoftmaxCrossEntropy()
            output_types = ['prediction', 'loss']
        super(GCNModelAngle, self).__init__(model,
                                       loss=loss,
    ...
    def _prepare_batch(self, batch):
        ...
        _, labels, weights = super(GCNModelAngle, self)._prepare_batch(
            ([], labels, weights))
